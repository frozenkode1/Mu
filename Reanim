--some stuff made by zachs, mostly edited by kode
...
local loadedAnimationid
local loadedposeData
local loadedjointMap
local loadedAnimationLength

local function loadFakeAnimation(loadAnimationid)
    --if loadedAnimationid == loadAnimationid then return end
    local success, loadedAnimation = pcall(function()
    	return game:GetObjects("rbxassetid://" .. loadAnimationid)[1]
    end)

    if not success or not loadedAnimation then
    	warn("Error loading animation with ID: " .. loadAnimationid)
    	return
    end
    ...
    local keyframes = loadedAnimation:GetKeyframes()
    table.sort(keyframes, function(a, b)
        return a.Time < b.Time
    end)
    
    task.spawn(function()
		    for _, joint in ipairs(ghostClone:GetDescendants()) do
        if joint:IsA("Motor6D") then
            jointMap[joint.Part1.Name] = joint
        end
    end
    ...
    for _, keyframe in ipairs(keyframes) do
        for _, pose in ipairs(keyframe:GetDescendants()) do
            if pose:IsA("Pose") then
                local motor = jointMap[pose.Name]
                if motor then
                    poseData[pose.Name] = poseData[pose.Name] or {
                        Motor = motor,
                        Frames = {}
                    }
                    table.insert(poseData[pose.Name].Frames, {
                        Time = keyframe.Time,
                        PoseCFrame = pose.CFrame
                    })
                end
            end
        end
    ...
    loadedAnimationLength = keyframes[#keyframes].Time
    loadedJointMap = jointMap
    loadedposeData = poseData
    loadedAnimationid = loadAnimationid
...
local function setGhostEnabled(newState)
	...
	loadedAnimationLength = nil
    	loadedJointMap = nil
    	loadedposeData = nil
    	loadedAnimationid = nil
...
local function playFakeAnimation(animationId)
    ...
    if fakeAnimRunning == true then
        stopFakeAnimation()
	if playingFakeAnimation then
	   playingFakeAnimation.Stop()
	end
    end
    wait()
    cloneSize = 1
...
local function PlayInterpolatedAnimation(Model, SpeedMultiplier, ghostOriginalMotorCFrames)
    SpeedMultiplier = fakeAnimSpeed
    local poseData = loadedposeData
    local jointMap = loadedjointMap
    local animationLength = loadedAnimationLength
    ...
    -- Function to perform binary search to find the two keyframes to interpolate between
    local function GetKeyframesForTime(frames, time)
        ...
        return frames[low - 1], frames[low]  -- Return the keyframes surrounding the time
    end

    local function GetLerpAlpha(t1, t2, current)
        local duration = t2 - t1
        return duration > 0 and ((current - t1) / duration) or 0
    end
    ...
    animationTask = task.spawn(function()
        ...
        local baseC0Cache = {}
        for _, data in pairs(poseData) do
            local motor = data.Motor
            local base = ghostOriginalMotorCFrames[motor]
            baseC0Cache[motor] = base and base.C0 or nil
        end
        ...
        while not ForceStop do
            ...
            if not Paused then
                TimePassed = TimePassed + (delta * SpeedMultiplier)
		if TimePassed >= animationLength then
                    TimePassed = 0
                end
            end
            ...
            for _, data in pairs(poseData) do
                ...
                local motor = data.Motor
                local baseC0 = baseC0Cache[motor]

                if baseC0 then
                    if #frames >= 2 then
                        local a, b = GetKeyframesForTime(frames, TimePassed)

                        if a and b then
                            local alpha = GetLerpAlpha(a.Time, b.Time, TimePassed)
                            local lerped = a.PoseCFrame:Lerp(b.PoseCFrame, alpha)

                            -- Apply the interpolation on top of the cached base.C0
                            motor.C0 = baseC0 * lerped
                        end
                    elseif #frames == 1 and TimePassed >= frames[1].Time then
                        motor.C0 = baseC0 * frames[1].PoseCFrame
                    end
                end
            end
        end
        ...
        for _, data in pairs(poseData) do
            ...
            local base = ghostOriginalMotorCFrames[motor]
            if base then
                TweenService:Create(motor, TweenInfo.new(0.25), {C0 = base.C0}):Play()
            end
        end
    end)
    ...
    return {
        Stop = function() forceStop = true fakeAnimSpeed = SpeedMultiplier task.cancel(animationTask) playingFakeAnimation = nil end,
        Pause = function() Paused = true end,
        Resume = function() Paused = false end,
        AdjustSpeed = function(speed) SpeedMultiplier = speed end,
    }
end


playingFakeAnimation = PlayInterpolatedAnimation(character, 1, ghostOriginalMotorCFrames)
...
local function createDraggableGui(getGhostEnabled, toggleGhost, getSizeValue, setSizeValue, getWidthValue, setWidthValue)
	...
	formattedValue = string.format("%.2f", value)
        speedLabel.Text = "Animation Speed: " .. formattedValue

	if playingFakeAnimation then
		playingFakeAnimation.AdjustSpeed(formattedValue)
        else 
        	fakeAnimSpeed = formattedValue
        end
    end
    ...
    local loadAnimButton = Instance.new("TextButton")
    loadAnimButton.Name = "loadAnimButton"
    loadAnimButton.Size = UDim2.new(0.4, 0, 0.4, 0)
    loadAnimButton.Position = UDim2.new(0.1, 0, 1, 0)
    loadAnimButton.Text = "Load Animation"
    loadAnimButton.BackgroundColor3 = Color3.fromRGB(70, 70, 70)
    loadAnimButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    local loadAnimButtonCorner = Instance.new("UICorner")
    loadAnimButtonCorner.CornerRadius = UDim.new(0, 8)
    loadAnimButtonCorner.Parent = loadAnimButton
    loadAnimButton.Parent = fakeAnimSection

    local fakeAnimButton = Instance.new("TextButton")
    fakeAnimButton.Name = "FakeAnimButton"
    fakeAnimButton.Size = UDim2.new(0.4, 0, 0.4, 0)
    fakeAnimButton.Position = UDim2.new(0.5, 0, 1, 0)
    ...
    loadAnimButton.MouseButton1Click:Connect(function()
        if ghostClone then
            local animId = fakeAnimTextBox.Text
            loadFakeAnimation(animId)
        else
            warn("No fake character available!")
       end
    end)
    fakeAnimButton.MouseButton1Click:Connect(function()
        if ghostClone and loadedAnimationid then

            playFakeAnimation()
        else
            warn("No fake character or loaded animation available!")
        end
    end)
